...
public class TesteExpressaoMaiorIgual {
	...
	public void testComparacaoInteiro(){
		...
		Assert.assertEquals(v_verdadeiro, new MaiorIgual(v3,v1).avaliar());
		Assert.assertEquals(v_verdadeiro, new MaiorIgual(v3,v2).avaliar());
		Assert.assertEquals(v_falso, new MaiorIgual(v1,v3).avaliar());
		Assert.assertEquals(v_verdadeiro, new MaiorIgual(v1,v2).avaliar());
	}
	...
	public void testeComparacaoTipo() {
		...
		Assert.assertEquals(Tipo.Booleano, new MaiorIgual(v1,v2).tipo());
		Assert.assertEquals(Tipo.Error, new MaiorIgual(v1,v3).tipo());
		Assert.assertEquals(Tipo.Error, new MaiorIgual(v3,v1).tipo());
		
	}
	
	@Test
	public void testeMaiorIgualPrint(){
		ValorInteiro v2 = new ValorInteiro(1);
		ValorInteiro v3 = new ValorInteiro(2);
		MaiorIgual maior = new MaiorIgual(v2,v3);
		PrettyPrinter pp = new PrettyPrinter();
		maior.aceitar(pp);
	}
	
	@Test
	public void testeMaiorIgualTamanho(){
		ValorInteiro v2 = new ValorInteiro(1);
		ValorInteiro v3 = new ValorInteiro(2);
		MaiorIgual maior = new MaiorIgual(v2,v3);
